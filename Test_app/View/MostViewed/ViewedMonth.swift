...
class ViewedMonth: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = viewTable.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! ViewedMonthCell
        let hero = pages[indexPath.row]
        let id = hero.id
        let publishedDateString = hero.published_date
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd"
        let publishedDate = dateFormatter.date(from: publishedDateString)
        guard let unwrappedPublishedDate = publishedDate else { return cell }
        let publishedMonth = MonthNY().calendar.component(.month, from: unwrappedPublishedDate)
        if MonthNY().month == publishedMonth {
            ...
            print("Поточний місяць не співпадає з місяцем 'published_date'")
        }
    return cell
    }
